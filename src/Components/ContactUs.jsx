...
export default function ContactUs(props) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('YOUR_BACKEND_API_ENDPOINT/sendEmail', {
        method: 'POST',
        headers: {
        ...
        <main className="bg-[#070A0B] w-full h-auto flex justify-between lg:flex-row flex-col gap-y-8" id="contact-main">
          <div className="px-[10px] md:px-[50px] w-full" id="touch-us">
            ...
            <form className="max-w-lg mx-auto py-3 text-white w-full" onSubmit={handleSubmit}>
              ...
              <button type="submit" className="w-full bg-[#E8D682] text-[#070A0B] font-semibold py-2 rounded hover:bg-[#D9C261] transition duration-300">
                Submit
              </button>
            </form>
          </div>
